...
(defmethod! screamer-score ((poly-object poly) 
                                                (domains t)
                                                (score-constraints t)
                                                &key (force-function "static-ordering linear-force") (random? t) (mcs-approx 2) (strategy :gfc)) 
  :initvals '(nil nil nil "static-ordering linear-force" t 2 :gfc) 
...
<KEY>: STRATEGY = :gfc for Generalized Forward Checking and :ac for Arc Consistency. Used for SCREAMER::FUNCALLV and SCREAMER::APPLYV." 
  :icon 487 
...
(defun midics-profiles-all (bpfs voices midics pitch-domain approx)
 (let* ((sample-all (multiple-value-bind (x y z) (om-sample (make-instance 'bpf-lib :bpf-list bpfs) (length (first pitch-domain))) (third (list x y z))))
          ...
          (min-max-positions (mapcar #'(lambda (sample) 
              	                        (let ((scale (om-scale sample 0 (1- (length all-midics)))))
               						     (om-round (x-append (car scale) (last-elem scale)))))
							  sample-min-max))			 			  
			  ;(om-round (butlast (cdr (om-scale sample 0 (1- (length all-midics))))))) sample-min-max))
...
(defmethod! constraint-measure ((constraints t)
   ...
   (let* ((chords? (list-of-listp voice-domain))
      ...
      (cond ((equal percent-cs-mode "exactly")
                 (let* ((test (if (and (= 1 fn-inputs-length) (not chords?))
                                  (mapcar #'(lambda (x) (apply (second score-constraints) (list x))) splitted-list) ;;;removed ?::mapcarv
                                  (mapcar #'(lambda (x) (apply (second score-constraints) x)) splitted-list)))
 				        (truesv (om?::sumv (mapcar #'?::reifyv test))) ;(mapcar #'s::count-truesv test))))
 						)
                 ...
                 (s::assert! (?::at-mostv (1- percent) #'(lambda (x) (s::=v 1 x)) truesv)) ;(s::assert! (s::<v truesv percent))
 				(setf *p-variables* (x-append test *p-variables*))  
 					))
                 ...
                 ((equal percent-cs-mode "greater-than")
                  ...
                  (if (= percent apply-length) 
                      (s::assert! (s::=v truesv percent))
                      (s::assert! (?::at-leastv (1+ percent) #'(lambda (x) (s::=v 1 x)) truesv))) ;(s::assert! (s::>v truesv percent))) 
 					(setf *p-variables* (x-append test *p-variables*))   
 					 ))
 ...
 (defun percentage-constraint-n-inputs-harmony (score-constraints voice-domain)
   (let* ((voice-length (length voice-domain))
      ...
      (cond ((equal percent-cs-mode "exactly")
                 (let* ((test (mapcar #'(lambda (x) (apply (second score-constraints) x)) splitted-list));;;changed ?::mapcarv to mapcar
                       (truesv (om?::sumv (mapcar #'?::reifyv test)))) ;(mapcar #'s::count-truesv test))))
                 (s::assert! (s::=v truesv percent)) 
 				(setf *p-variables* (x-append test *p-variables*))   
 					))
                 ...
                 (s::assert! (?::at-mostv (1- percent) #'(lambda (x) (s::=v 1 x)) truesv)) ;(s::<v truesv percent))
 				(setf *p-variables* (x-append test *p-variables*))  
 				))
...
(defun build-screamer-score-domain (voices domains mcs-approx random?)
 ...
 (list pitch-variables-without-rests
	   all-chords pitch-durs-domain
	   pitch-onset-domain
