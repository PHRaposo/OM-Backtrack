...
(defclass non-deter-window (EditorWindow);(editordindow)
          ...
          (cond 
           ((omclass-p (class-of (class-of value)))
            (setf dialog (om-make-window 'non-deter-window 
                           ...
                           :back-color (om-make-color 0.875 0.875 0.875)))
            (cond
...
#|
;;; OM5
              (om-make-edit-view 'om-text-edit-view
                                                    :save-buffer-p t
                                                    :scrollbars :v
                                                    :text (format nil "~D" value)
                                                    :wrap-p t
...
;;; OM4
              (make-instance 'scrolling-fred-view
                :save-buffer-p t
                :h-scrollp nil
                :dialog-item-text (format nil "~D" value)
                :v-scrollp t
                :wrap-p t
                :view-size (make-point (- (w dialog) 25) (- (h dialog) 50))
                :view-position #@(25 35)))))
				; :text	;:scrollbars :v ))))
					;:save-buffer-p t
					;:wrap-p t))))
